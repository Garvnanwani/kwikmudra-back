...
exports.userprofile = asyncHandler(async (req, res, next) => {
  const { avatar, username, fullname, email, _id, phoneNo } = req.user

  res.status(200).json({
    success: true,
    data: { avatar, username, fullname, email, _id, phoneNo },
  })
})
...
exports.resetPassword = asyncHandler(async (req, res, next) => {
  crypto.randomBytes(32, async (err, buffer) => {
    ...
    try {
      ...
      sendMail(
        email,
        `<p>You requested for password change</p><h5>click on this <a href="https://instagraam.netlify.app/reset/${token}"> link </a> to reset your password</h5>`
      )
        .then(() => {
